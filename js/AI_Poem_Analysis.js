...
function poemAnalysis(poem, div_id) {
    ...
    d3.json("processeddata/VAD.json").then(function (sentiment) {
        ...
        let LegendColors = [["#ffa1a5", "#a3ffba"], ["#fff2db", "#ffc359"], ["#ffeea1", "#ffa1a5"]];
        Cat.forEach(function (category, i) {
            ...
            document.getElementById("user" + category).innerHTML = "<p class='poemtext'><br>" + labeledpoem + "</p><p class='poemtext'><br>Color Notation: <br>0<span style='width: 50px; margin: 0 5px; line-height: 5px !important; color: transparent; background: linear-gradient(90deg, " + LegendColors[i][0] + " 0%, " + LegendColors[i][1] + " 100%);'> ---------------------- </span>1</p>";
        });
